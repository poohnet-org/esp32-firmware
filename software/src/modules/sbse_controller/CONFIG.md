...
| `kp_milli` | `1000` (= Kp 1.0) | Kp × 1000, 100…2000 | **Proportional controller gain.** Per tick: `new_setpoint = battery_now + Kp · (ema_grid − target_grid_w)`. `Kp = 1.0` ≈ "one-shot correction." Lower (~0.5) = slower, more damped. Higher (~1.5) = snappier but can ring against the EMA filter. |
...
each tick (default 300 ms, gated by `enabled` + connection + `paused`):
  ...
  raw_setpoint   = battery_now + Kp · delta
